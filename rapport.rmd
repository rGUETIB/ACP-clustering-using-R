...
Concernant les inégalités sociales, on comprends donc que l'Argentine est caractérisée par des provinces développées et d'autres qui le sont moins.
...
L'accès à l'éducation est un critère de développement d'un pays, d'une région ou d'une province. Nous pourrions donc prendre en compte la variable "**illiteracy**" qui rendrait compte, dans une province où ce taux est élevé, d'un faible niveau de développement. 
...
De même, nous devons nous assurer que la réduction des dimensions est possible et pertinente. Pour cela, nous utilisons un test statistique : le test de Bartlett. Si l'hypothèse nulle (H0) -qui est qu'il n'y a pas de lien, pas de corrélation entre toutes les variables- est rejeté alors cela signifie qu'au contraire, il y a bien une relation entre elles.
...
Superposons maintenant nos individus sur nos variables ainsi représentées dans un graphique. Cela nous permettra notamment d'avoir une idée des groupes que nous pouvons former. 
...
Maintenant que nous avons choisi nos deux axes, étape primordiale pour réaliser notre étude, rentrons dans le vif du sujet : qu'elles sont les provinces les plus développées et qu'elles sont celles qui ne le sont pas (ou qui le sont moins) ? 
Pour répondre à cette question, nous utiliserons des algorithmes de partionnement et deux en particuliers : celui des K-means et le Custering Ascendant Hierarchique (CAH). Nous verrons que ces deux algorithmes produisent le même résultat, les mêmes regroupements ce qui rend compte de la robustesse de nos modèles.
...
L'algorithme des K-means nous permet alors de voir que le cluster n°1 est le groupe d'individus le plus vulnérable et le cluster n°2 est le groupe d'individus le plus développé. C'est bien l'intuition que nous avions eue plus haut. 
*NB : nous aurions pu faire plus de deux groupes et utiliser des outils de visualisation (tel que l'indice "silhouette", le "coude d'effondrement" ou encore l'écart statistique) pour nous aider à déterminer le nombre optimal de cluster à faire. Ce n'est pas ce que nous avons fait, car l'objet de notre étude est de faire __deux__ groupes pour ensuite extraire parmi l'un des deux les deux provinces les moins développées et parmi l'autre les deux provinces les plus développées. Notons toutefois que ces indicateurs concernant le nombre de cluster idéal nous signal aussi de faire 2 groupes (cf. annexe n°5)*
...
- On constate alors que nous avons bien un groupe de province développée caractérisé par un taux de pauvreté assez élevé (13%) ; un taux de personnes n'ayant pas accès aux soins de santé assez élevé puisque plus de la moitié de la population de ces provinces sont dans ce cas ; un PIB/habitant plus faible (9$ par habitant), et un taux de population sans instruction assez élevé (environ 5%).
...
On voit bien ici que, quelle que soit la méthode de clustering choisie (k-means ou CAH), nous arrivons au même résultat : deux groupes composés des mêmes individus et ayant les mêmes caractéristiques (développés *versus* peu développés). Cela prouve la robustesse de notre modèle.
...
Il en est de même si l'on se base sur le second score de richesse que nous avons créé qui correspond à une moyenne, individus par individus, des deux indicateurs de développement que nous avons retenus.
...
De même ici, que l'on se base sur notre premier indicateur de pauvreté constitué de l'addition de notre 3 variables caractéristiques de la vulnérabilité (province par province) ou sur notre second indicateur qui est une moyenne de ces derniers (province par province également), nous arrivons à la même conclusion : ce sont les provinces de **Chaco** et **Formosa** qui sont les provinces les plus vulnérables parmi le groupe de province les plus vulnérable d'Argentine. 
...
Quant au <mark>programme "No-Poor", il devrait plutôt être mis en place dans les provinces de **Santa Cruz** et **Neuquen**, provinces développées</mark>. Pour rappel, ce dernier permettrait alors aux personnes y résidents de ne pas tomber sous le seuil de pauvreté.
...
Ceci rend donc compte du fait que le choix des indicateurs n'est pas neutre, il permet de dire telle chose et pas telle autre. Quoiqu'il en soit, si l'on se base sur les indicateurs que nous avons à notre disposition, nos résultats sont ceux cités plus haut. Et tout porte à croire que nos conclusions sont cohérente et robustes puisque sont similaires quelle que soit la méthode de partitionnement adoptée.
